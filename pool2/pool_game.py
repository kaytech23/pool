...
class Game(pyglet.window.Window):
    ...
    def on_key_press(self, symbol, modifiers):
        if symbol == key.SPACE and (self.model.state == pool2.pool_learner_no_gui.GameState.GameOver
                                    or self.model.state == pool2.pool_learner_no_gui.GameState.Start):
            self.model.init_game()
            self.model.state = pool2.pool_learner_no_gui.GameState.PlayerMove
